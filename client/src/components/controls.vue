<template>
  <ul>
    ...
    <!-- Hidden input for keyboard triggering -->
    <input
      ref="hiddenKeyboardInput"
      type="text"
      class="hidden-keyboard-input"
      @blur="onKeyboardInputBlur"
    />
    <!-- Keyboard toggle button -->
    <li>
      <i
        :class="['fas', 'fa-keyboard', 'keyboard-toggle', { active: keyboardVisible }]"
        v-tooltip="{
          content: $t('controls.keyboard_toggle') || 'Show/Hide Keyboard',
          placement: 'top',
          offset: 5,
          boundariesElement: 'body',
          delay: { show: 300, hide: 100 },
  ...
  export default class extends Vue {
    ...
    toggleKeyboard() {
      ...
      if (this.keyboardVisible) {
        this.$nextTick(() => {
          const input = this.$refs.hiddenKeyboardInput as HTMLInputElement
          if (input) {
            input.focus()
            input.click()
          }
