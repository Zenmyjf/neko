    ...
    <div ref="player" class="player">
      <div ref="container" class="player-container">
        ...
        <textarea
          ...
          @touchmove.stop.prevent="onTouchDragMove"
          @touchstart.stop.prevent="onTouchDragStart"
          @touchend.stop.prevent="onTouchDragEnd"
...
<script lang="ts">
  ...
  export default class extends Vue {
    ...
    sendRelativeMouseMove(deltaX: number, deltaY: number) {
      this.$client.sendData('mousemove', {
        x: Math.round(deltaX),
        y: Math.round(deltaY),
      })
    ...
    onTouchDragStart(e: TouchEvent) {
      ...
      this.lastTouchX = first.clientX
      this.lastTouchY = first.clientY
      this.touchDragging = true

      // Send initial mousedown for the first touch
    ...
    onTouchDragMove(e: TouchEvent) {
      if (!this.hosting || this.locked || !this.touchDragging) {
        return
      }
      ...
      const deltaX = first.clientX - this.lastTouchX
      const deltaY = first.clientY - this.lastTouchY

      // Scale delta to remote resolution
      const scaledDeltaX = (w / rect.width) * deltaX
      const scaledDeltaY = (h / rect.height) * deltaY

      // Send relative mouse movement
      this.sendRelativeMouseMove(scaledDeltaX, scaledDeltaY)
